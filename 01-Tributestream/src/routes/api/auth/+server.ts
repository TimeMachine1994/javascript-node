 ...
 *      (/wp-json/tributestream/v1/user-cap) to retrieve the user's roles & capabilities.
 *   4. Return the combined data (token, user info, roles, capabilities) in the JSON response.
 *
 * Note: This endpoint is written in TypeScript for SvelteKit 5.
 */
...
export async function POST({ request }: RequestEvent) {
    ...
    try {
        ...
        if (!wpAuthResponse.ok) {
            ...
            return json({ message: wpAuthData.message }, { status: wpAuthResponse.status });
        }
    } catch (error) {
